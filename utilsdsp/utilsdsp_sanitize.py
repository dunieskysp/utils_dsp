...
def truncate_filename(filename: str, chars_cant: int = 120) -> str:
    """
    Truncar el nombre del fichero o directorio.

    Parameters:
    filename (str): Nombre del fichero o directorio.
    chars_cant (int) [Opcional]: Cantidad de carácteres a dejar en el nombre.

    Returns:
    srt: Devuelve el nombre truncado o no, según los carácteres que contenga.
    """

    # Retornar el nombre tal y como está, sí tiene menos carácteres de los admitidos.
    ...
    # Indice donde vamos a truncar el nombre del fichero.
    idx_trunc = chars_cant - len(ext)

    # Retornar el nombre truncado (A los ficheros agregarle tres puntos al final).
    return f'{name[:idx_trunc - 3]}...{ext}' if ext else name[:idx_trunc]


def sanitize_filename(filename: str, chars_cant: int = 120) -> str:
    """
    Sanear el nombre de un fichero o directorio, eliminando
    los carácteres no adminitidos por Linux y Windows.

    Parameters:
    filename (str): Nombre del fichero o directorio.
    chars_cant (int) [Opcional]: Cantidad de caracteres a dejar en el nombre.
